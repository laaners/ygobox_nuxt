<template>
	<div class="flex-col" style="align-items: start">
		<table style="border-spacing: 0; border-collapse: collapse">
			...
			<tbody>
				<tr
					v-for="(card, i) of bannedCards.cards.slice(21*index,21*(index+1))"
					:key="i"
...
export default {
	...
	mounted() {
		const max = JSON.parse(JSON.stringify(this.bannedCards.cards)).sort((a,b) => b.name.length-a.name.length)
