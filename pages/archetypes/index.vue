...
export default {
  ...
  methods: {
    next() {
      this.counter += 20;
      this.archetypeList = this.allArchetypes.slice(this.counter, this.counter+20);
    },
    prev() {
      this.counter -= 20;
      this.archetypeList = this.allArchetypes.slice(this.counter, this.counter+20);
    ...
    focusOn(mechanic) {
      if(mechanic === 'Pendulum') {
        ...
        this.archetypeList = this.allArchetypes.filter((arc) => {
          let max = 0;
          for(const key in arc.focus) {
            if(arc.focus[key] > max && key !== 'No Extra' && key !== 'Pendulum') max = arc.focus[key];
          }
          return arc.focus[mechanic] > 1 && arc.focus[mechanic] === max;
        });
      }
